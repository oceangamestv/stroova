...
**Очистка логов PM2 (если занимают много места):** на сервере выполни `pm2 flush` — все файлы в `~/.pm2/logs/` будут очищены (out и err для stroova-api, stroova-telegram-bot и т.д.). Проверить размер: `du -sh ~/.pm2/logs`.
...
**Список слов без озвучки и генерация на локальном ПК:** в админке есть раздел **«Аудио файлы»** (Админ → Аудио файлы). Там можно запустить **полную проверку** (сканирование каталогов на сервере и обновление флагов в БД) или **только новые слова** (проверяются только слова, для которых флаги ещё не проставлены). После проверки отображается список слов без озвучки и кнопка **«Скачать список для генерации»** — сохраняется файл `missing-audio.json`. На локальном ПК выполни: `node server/generate-audio.mjs missing-audio.json` — скрипт сгенерирует только недостающие WAV (оба голоса: female и male). Затем залей новые файлы в `public/audio/female/` и `public/audio/male/` на сервере по FTP/SCP и при необходимости пересобери фронт.

### 4. Проверка

Открой сайт в браузере и убедись, что всё работает. Логи API: `pm2 logs stroova-api` (выход — Ctrl+C).

**Если Telegram-бот в PM2 часто перезапускается** (в списке PM2 у `stroova-telegram-bot` большой счётчик перезапусков), причину смотри в логах на сервере:
- **Только ошибки:** `pm2 logs stroova-telegram-bot --err --lines 300`
- **Файл логов:** `tail -200 ~/.pm2/logs/stroova-telegram-bot-error.log`
- **Инфо о процессе:** `pm2 show stroova-telegram-bot` (последние перезапуски, время работы)

**Ошибка 409 «Conflict: only one bot instance»:** Telegram разрешает только одно соединение getUpdates на один токен. Если бот падает и PM2 сразу перезапускает его, новый процесс конфликтует со старым сеансом → снова 409 → цикл перезапусков. Что сделать: (1) Убедиться, что бот запущен **только в одном месте** — один процесс в PM2 на этом сервере и нигде больше (другой сервер, другой пользователь, ручной запуск). Проверка: `pm2 list | grep telegram` — должна быть одна строка `stroova-telegram-bot`. (2) В коде добавлена обработка 409: при такой ошибке процесс выходит с задержкой 15 с, в PM2 для бота задан `restart_delay: 15000`, чтобы после падения новый старт был через 15 с и конфликт успевал сняться. После деплоя обновлённого кода перезапусти бота один раз: `pm2 restart stroova-telegram-bot`, затем `pm2 save`.

Другие частые причины: неверный/отозванный `TELEGRAM_BOT_TOKEN`, сетевые ошибки (ETIMEDOUT до api.telegram.org). В коде бота добавлено логирование необработанных ошибок — при падении в логах будет видна причина.
...
**Порядок миграций:** 001 → 002 → 003_active_days_and_rewards → 003_add_dictionary_version → 004 → 005 → 006 → 007 → 008. Миграции идемпотентны (`IF NOT EXISTS` и т.д.), повторный прогон безопасен.
