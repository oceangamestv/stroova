import type { Accent, Word } from "./contracts/types";

export const A0_DICTIONARY: Word[] = [
...
export const getDictionary = () => A0_DICTIONARY;

export const getWordById = (id: number): Word | undefined =>
  A0_DICTIONARY.find((w) => w.id === id);
...
export const getWordsByAccent = (accent: Accent) => {
  if (accent === "both") return A0_DICTIONARY;
  return A0_DICTIONARY.filter((word) => word.accent === accent || word.accent === "both");
};

export const getRandomWords = (count: number, accent: Accent = "both") => {
  const words = getWordsByAccent(accent);
  const shuffled = [...words].sort(() => Math.random() - 0.5);
  return shuffled.slice(0, Math.min(count, words.length));
};

export const findWordByEnglish = (text: string) =>
  A0_DICTIONARY.find((word) => word.en.toLowerCase() === text.toLowerCase());

export const findWordByRussian = (text: string) =>
  A0_DICTIONARY.find((word) => word.ru.toLowerCase() === text.toLowerCase());
