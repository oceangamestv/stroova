...
Инструкция: домен в **Reg.ru** — нужно направить его на IP твоего VPS и настроить сервер. **Текущий боевой сайт:** https://stroova.ru. Ниже пример с Jino; для другого хостинга подставь свой IP.
...
**A-запись в Reg.ru** должна указывать на IP твоего VPS. Узнать IP можно так:

**Вариант А — из панели хостинга**

В панели управления VPS (Nuxt, Jino и т.д.) на странице сервера указан **IP-адрес**. Скопируй его для Reg.ru.

**Вариант Б — с твоего компьютера**

В PowerShell: `ping stroova.ru` — в ответе будет строка «Ответ от X.X.X.X», это IP сервера. Запиши его для Reg.ru.
...
5. **Добавь A-запись для основного домена:**
   ...
   - Значение / IP: **IP твоего сервера** (узнать: панель хостинга или `ping stroova.ru`).
   - Сохрани.
6. **Добавь запись для www** (чтобы работало `https://www.example.ru`):
   - Тип: **A**
   - Поддомен: **www**
   - Значение: **тот же IP** (что и для @).
   - Сохрани.
...
Найди строку с `server_name` (например с IP). Добавь домен через пробел:
...
Если такой строки нет — добавь её. Если в .env указан старый адрес — замени на `https://stroova.ru/api`. Сохрани: **Ctrl+O**, Enter, выход: **Ctrl+X**.
...
| Шаг | Где | Действие |
|-----|-----|----------|
| 1 | Хостинг / ping | Узнать IP: панель хостинга или `ping stroova.ru` |
| 2 | Reg.ru | A-запись @ → IP сервера, A-запись www → тот же IP |
