...
- В **.env** при сборке должен быть задан **VITE_API_URL** — URL вашего API (боевой сервер: `https://147.45.196.155/api` или ваш домен), чтобы приложение на телефоне подключалось к серверу

## Быстрая сборка APK

### 1. URL бэкенда

- **Боевой APK:** URL задаётся в `.env.production` (по умолчанию боевой сервер `https://147.45.196.155/api` или ваш домен).
